...
class TestUrls():
    ...
    def test_not_logged_in(self, url, status_code, clients):
        ...
        assert resp.url.endswith("/auth/google/login/?next=/")
    ...
    def test_button_update_global(self, clients):
        ...
        resp = clients['c_in'].generic(
            'POST', reverse('button_update_global'), json.dumps(post_data), content_type='application/json'
        )
        assert resp.status_code == 200
    ...
    def test_button_update_local(self, clients):
        ...
        resp = clients['c_in'].generic(
            'POST', reverse('button_update_local'), json.dumps(post_data), content_type='application/json'
        )
        assert resp.status_code == 200
    ...
    def test_push_auto_button(self, clients):
        ...
        resp = clients['c_in'].generic(
            'POST', reverse('button_update_sync'), json.dumps(post_data), content_type='application/json'
        )
        assert resp.status_code == 200
    ...
    def test_email_field(self, clients):
        ...
        resp = clients['c_in'].generic(
            'POST',
            reverse('api_group', kwargs={'pk': geneva.pk}),
